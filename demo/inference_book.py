...
def parse_txt_script(txt_content: str) -> Generator[Tuple[List[str], List[str]], None, Tuple[List[str], List[str]]]:
    """
    Parse txt script content and extract speakers and their text, yielding a tuple
    of scripts and speaker numbers for each paragraph separated by a blank line.
    ...
    for line in lines:
        ...
        # Check for empty line first to yield the previous section
        if not line:
            # Yield the accumulated scripts and speaker numbers
            # This handles paragraphs separated by empty lines
            if current_speaker and current_text:
                scripts.append(f"Speaker {current_speaker}: {current_text.strip()}")
                speaker_numbers.append(current_speaker)
            
            # Yield the complete paragraph and then reset for the next one
            if scripts:
                yield scripts, speaker_numbers
        ...
        # This part processes each non-empty line
        line = 'Speaker 1: ' + line
        ...
        if match:
            ...
            # Continue text for the current speaker
            if current_text:
                current_text += " " + line
            else:
                current_text = line
    
    # Don't forget to yield the last segment after the loop ends
    if current_speaker and current_text:
        scripts.append(f"Speaker {current_speaker}: {current_text.strip()}")
        speaker_numbers.append(current_speaker)
...
def parse_args():
    ...
    parser.add_argument(
        "--device",
        type=str,
        default=device if torch.cpu.is_available() else device,
...
def main():
    ...
    script_generator = parse_txt_script(txt_content)
    ...
    for index, tuple_item in tqdm(enumerate(script_generator)):

        scripts, speaker_numbers = tuple_item
        print("\n scripts :", scripts, "\n")

        txt_filename = os.path.splitext(os.path.basename(args.txt_path))[0]
        output_path = os.path.join(args.output_dir, f"{txt_filename}_{index}.wav")
        ...
        print(f"scripts size {len(scripts)}")
        # Prepare data for model
        full_script = '\n'.join(scripts)

        full_script = replace_chars(full_script, char_rep_map)
